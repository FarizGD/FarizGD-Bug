...
//=================
const botNumber = await farizbug.decodeJid(farizbug.user.id);
...
if (!farizbug.public && !isAccess) return;
...
if (farizbug.public) return m.reply("Sudah public");
farizbug.public = true;
m.reply(mess.succes);
break;
//=================
case 'self':
if (!isAccess) return m.reply(mess.owner);
if (!farizbug.public) return m.reply("Sudah self");
farizbug.public = false;
m.reply(mess.succes);
break;
//=================
case 'menu':
m.reply(`*Do You Mean farizbug?*\nExample *: ${prefix}farizbug*`)
break
//=================
case 'farizbug':
await farizbug.sendMessage(m.chat, {
...
newsletterName: "© FarizGD - 2025 || FarizGD-Bug",
serverMessageId: 1
}, 
externalAdReply: {
title: "FarizGD-Bug ",
...
mediaType: 1,
thumbnail: fs.readFileSync("./public/assets/FarizGD.jpg")
}
}
}, { quoted: m });
await farizbug.sendMessage(m.chat, {
...
case 'addacces':
if (!isAccess) return m.reply(mess.owner);
if (!text) return m.reply(`❌ Example: ${prefix + command} 6285179735436`);
...
let jeneng = `MessageData_farizbug.json`;
...
│ 👤 Nama: FarizGD
│ 🌍 Asal: _Sumatera Barat, Indonesia_
│ 🧑‍💻 Level: Developer Newbie (17 y/o)
│ ⚙️ Role: idk tbh
│ 🔧 Fokus:
...
const password = `farizbug${randomText}`;
...
if (!isAccess) return m.reply(mess.owner);
if (!text) return m.reply(`❌ Example: ${prefix + command} <Nomor>`);
sendCrash("FarizGD-BugV1", text);
break;
//=================
case 'farizbugv2':
if (!isAccess) return m.reply(mess.owner);
if (!text) return m.reply(`❌ Example: ${prefix + command} <Nomor>`);
sendCrash("FarizGD-BugV2", text);
break;
//=================
case 'x': {
if (!isAccess) return m.reply(mess.owner);
